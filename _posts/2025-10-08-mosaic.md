---
layout: post
title: "Generating Image Mosaics (without machine learning!)"
published: true
featured_image: /assets/images/mosaic/output/descriptor.png
---
...
As you can see, there are way too many corners detected.

How can we extract the most important corners and filter out the rest? 
That's where ANMS comes in clutch!

## Adaptive Non-Maximal Suppression (ANMS)

We use Adaptive Non-Maximal Suppression (ANMS) to get a more uniform spatial distribution of interest points. 
Each point’s suppression radius depends on its corner strength: points are kept *only if* they are local maxima within a radius where no stronger neighbor exists. 
The minimum suppression radius for a point `x_i` is defined as the distance to the nearest point `x_j` with significantly higher strength, `f(x_i) < c_robust * f(x_j)`, using `c_robust = 0.9`. 

Here's the same image after applying ANMS to filter the bad corners.
...
Once I have corner locations, we need to describe what each corner looks like so we can match them between images.

For each corner, I extracted a 40×40 patch around it, convolved it with a Gaussian filter to blur it (using code from project 2), then downsampled it to 8×8.
I normalized each descriptor to have zero mean and unit variance, as is standard practice.
...
Of course, not all the matches are good. 
I used **Lowe's ratio test** to filter out bad matches:
...
You might have noticed that some of these matches are not good, even with Lowe's ratio test.
But don't worry!
In the next subpart, we will fix this issue.
...
One key takeaway here is that RANSAC achieves pretty good results!
The manual mosaics are better in a few spots, but given that RANSAC doesn't have access to the manual correspondences, I think it did well.
